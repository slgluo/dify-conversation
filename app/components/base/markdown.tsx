...
const Link = ({ node, ...props }: any) => {
  ...
  // hide .bin file download link
  else {
    return <a {...props} style={{ display: node.children[0]?.value.indexOf('.bin') ? 'none' : 'block' }} target="_blank" className="underline decoration-dashed !decoration-primary-700 cursor-pointer">{node.children[0] ? node.children[0]?.value : 'Download'}</a>
  }
}
